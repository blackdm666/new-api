...
Responses protocol media uses host-injected `ctx.artifacts[key].url`. OpenAI Video renderers may return provider fields and URLs from `task.data`; the host preserves those extensions and metadata, but overwrites `id`, `object`, `model`, `status`, `progress`, `created_at`, and `completed_at`. This fork also sets legacy `task_id` to the public `id` on create and retrieve; upstream API v1 removes this legacy field. Renderers must return a JSON object. Returning `task.data` exposes the latest persisted upstream snapshot, subject to the host's task-ID replacement and inline-video redaction; it does not perform a fresh upstream query or forward HTTP headers/status codes.

This fork's host-owned `POST /v1/video/generations` route explicitly bridges to the same `openai_video` create protocol as `/v1/videos`, preserving ordinary-channel fallback. Plugins still declare `openai_video`, not a duplicate native route. A decoded `requestBody.callback_url` string enters the host's existing URL validation and durable callback delivery; it should not be forwarded to the provider. These compatibility bridges require the fork host and are not guaranteed by the upstream v1 manifest. No additional manifest fields or hooks are required.
