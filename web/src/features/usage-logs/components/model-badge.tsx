...
function ModelBadgeContent(props: ModelBadgeProps & { copyable: boolean }) {
  let provider = resolveModelProvider(props.modelName)
  const model = props.modelName.toLowerCase()
  // Keep storefront product icons without duplicating provider categorization.
  if (model.includes('codex')) {
    provider = { name: 'OpenAI', label: 'Codex', icon: 'Codex.Color' }
  } else if (
    ['seedance', 'seedream', 'seededit'].some((name) => model.includes(name)) ||
    /^sd2(?:[.\-\s]|$)/.test(model)
  ) {
    provider = { name: 'Doubao', label: 'Seedance', icon: 'Jimeng.Color' }
  } else if (model.includes('veo-')) {
    provider = { name: 'Gemini', label: 'Vertex AI', icon: 'VertexAI.Color' }
  }
