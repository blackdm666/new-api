...
export function pricingFromDraft(data: ModelRatioData): PricingValues {
  let mode = 'ratio'
  if (data.billingMode === 'tiered_expr') mode = 'tiered_expr'
  else if (data.billingMode === 'per-second') mode = 'per_second'
  ...
  for (const [field, key] of Object.entries(pricingFieldMap)) {
    ...
    if (
      !isCompletePricingNumber(String(value)) ||
...
export function applyPriceSyncSelections(
  ...
  for (const [name, fields] of Object.entries(selections)) {
    for (const [key, value] of Object.entries(fields)) {
      if (key === 'billing_mode' || key === 'billing_expr') continue
      if (!isCompletePricingNumber(String(value))) {
        throw new Error(t('Enter a finite, non-negative price'))
      }
    }
    ...
    if (expression) {
      ...
      if (fixed) {
        const currentModes = JSON.parse(
          result['billing_setting.billing_mode']
        ) as Record<string, string>
        const selectedMode = fields.billing_mode || currentModes[name]
        if (selectedMode === 'per_request' || selectedMode === 'per_second') {
          next['billing_setting.billing_mode'] = selectedMode
        }
      }
    ...
    validated['billing_setting.billing_mode'] =
