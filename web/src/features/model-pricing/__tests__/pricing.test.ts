...
describe('shared model pricing', () => {
  it.each(['per_request', 'per_second'])(
    'round-trips explicit %s prices without losing the billing unit',
    (mode) => {
      const values = { ModelPrice: 0, 'billing_setting.billing_mode': mode }
