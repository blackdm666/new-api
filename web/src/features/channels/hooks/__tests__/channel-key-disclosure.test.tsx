...
function Harness(props: {
  open: boolean
  channelId: number
  kind?: 'key' | 'balance-query-token'
}) {
  const disclosure = useChannelKeyDisclosure(
    props.open,
    props.channelId,
    props.kind
  )
...
function channelVerification(scope = 'channel.key.read') {
...
it('requires a channel-bound proof for balance-query token disclosure', async () => {
  const proof = channelVerification('channel.balance_token.read')
  const post = vi.spyOn(api, 'post').mockImplementation(async (url) => {
    if (url === '/api/verify') return proof
    if (url === '/api/channel/123/balance_query/token') {
      return { data: { success: true, data: { token: 'BALANCE_TOKEN' } } }
    }
    throw new Error(`Unexpected POST ${url}`)
  })
  const user = userEvent.setup()
  render(<Harness open channelId={123} kind='balance-query-token' />)
  ...
  expect(post).toHaveBeenCalledWith(
    ...
    expect.objectContaining({
      scope: 'channel.balance_token.read',
      context: { channel_id: 123 },
    }),
    expect.anything()
  )
  expect(post).toHaveBeenCalledWith(
    '/api/channel/123/balance_query/token',
    undefined,
    expect.objectContaining({
      headers: { 'X-Security-Proof': 'channel-proof' },
    })
  )
})
