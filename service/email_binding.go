...
func sendEmailBindingCodes(state model.EmailBindingState, codes emailBindingCodes) error {
	...
	if err := common.SendEmailWithProfile(common.SMTPProfileSecurity, subject, state.Email, content); err != nil {
		return err
	}
	if codes.Old != "" {
		...
		return common.SendEmailWithProfile(common.SMTPProfileSecurity, subject, state.CurrentEmail, content)
	}
	return nil
}
