...
// A host may reject float decoding; the compatibility path must respect
// that codec decision instead of bypassing it via encoding/json.
type floatRejectingCodec struct{ hostJSONCodec }

func (floatRejectingCodec) Unmarshal(data []byte, target any) error {
	if _, ok := target.(*float64); ok {
		return fmt.Errorf("float decoding disabled by host codec")
	}
	return json.Unmarshal(data, target)
}

func TestResponsesTimestampCompatibilityUsesHostCodec(t *testing.T) {
	for _, test := range []struct {
		input   string
		want    int
		invalid bool
	}{
		{`{"created_at":1780000000}`, 1780000000, false},
		{`{"created_at":1780000000.75}`, 1780000000, false},
		{`{"created_at":"1780000000"}`, 1780000000, false},
		{`{"created_at":1e30}`, 0, true},
	} {
		var response dto.OpenAIResponsesResponse
		err := Unmarshal([]byte(test.input), &response)
		if test.invalid {
			require.Error(t, err)
			continue
		}
		require.NoError(t, err)
		assert.EqualValues(t, test.want, response.CreatedAt)
	}
	kitutil.SetCodec(floatRejectingCodec{})
	t.Cleanup(func() { kitutil.SetCodec(hostJSONCodec{}) })
	var response dto.OpenAIResponsesResponse
	assert.Error(t, Unmarshal([]byte(`{"created_at":1780000000.75}`), &response))
}
