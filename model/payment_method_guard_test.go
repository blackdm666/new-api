...
func TestRechargeEpayCreditsQuotaExactlyOnce(t *testing.T) {
	...
	alreadyDone, err := RechargeEpay(order.TradeNo, "alipay", order.Money, "127.0.0.1")
...
func TestRechargeEpayUpdatesPaymentMethodToActual(t *testing.T) {
	...
	alreadyDone, err := RechargeEpay(order.TradeNo, "wxpay", order.Money, "127.0.0.1")
	require.NoError(t, err)
	assert.False(t, alreadyDone)
...
func TestRechargeEpayRejectsVerifiedAmountMismatchBeforeCredit(t *testing.T) {
	truncateTables(t)
	...
	user := insertUserForPaymentGuardTest(t, 506, 9)
	order := createEpayTestOrder(t, user.Id, "EPAYTESTAMOUNT", PaymentProviderEpay, common.TopUpStatusPending)

	_, err := RechargeEpay(order.TradeNo, "alipay", 9.99, "127.0.0.1")
	require.ErrorIs(t, err, ErrTopUpAmountMismatch)
	assert.Equal(t, 9, getUserQuotaForPaymentGuardTest(t, user.Id))

	reloaded := GetTopUpByTradeNo(order.TradeNo)
	require.NotNil(t, reloaded)
	assert.Equal(t, common.TopUpStatusPending, reloaded.Status)
	assert.InDelta(t, 10.0, reloaded.Money, 0.000001)
}
...
func TestRechargeEpayRejectsForeignAndNonPendingOrders(t *testing.T) {
	...
	t.Run("order from another payment provider", func(t *testing.T) {
		...
		_, err := RechargeEpay(order.TradeNo, "alipay", order.Money, "127.0.0.1")
	...
	t.Run("missing order", func(t *testing.T) {
		_, err := RechargeEpay("EPAYTESTMISSING", "alipay", 10, "127.0.0.1")
