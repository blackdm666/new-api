...
func validateModelPricing(name string, values, previous PricingValues) error {
	...
	for key, value := range values {
		...
		if key == "billing_setting.billing_mode" {
			if value != billing_setting.BillingModeRatio && value != billing_setting.BillingModeTieredExpr && value != billing_setting.BillingModePerRequest && value != billing_setting.BillingModePerSecond {
	...
	mode := values["billing_setting.billing_mode"]
	if mode == billing_setting.BillingModePerRequest || mode == billing_setting.BillingModePerSecond {
		if _, exists := values["ModelPrice"]; !exists {
			return errors.New("fixed billing requires a model price")
		}
	}
	if mode == "tiered_expr" {
