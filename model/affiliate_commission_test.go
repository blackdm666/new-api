...
func TestAffiliateUpgradeCandidateApprovalAndNotificationRetry(t *testing.T) {
	...
	failedNotices, total, err := ListFailedAffiliateUpgradeNotices(&common.PageInfo{Page: 1, PageSize: 10})
	require.NoError(t, err)
	assert.Equal(t, int64(1), total)
	require.Len(t, failedNotices, 1)
	assert.Equal(t, inviter.Username, failedNotices[0].InviterUsername)
