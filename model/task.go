...
type TaskBillingContext struct {
	...
	BillingUnit     string                       `json:"billing_unit,omitempty"`      // 固定价格单位：request / second
...
func (Task *Task) InsertWithCallback(callbackURL string) error {
	return Task.InsertWithCallbackContext(context.Background(), callbackURL)
}

func (Task *Task) InsertWithCallbackContext(ctx context.Context, callbackURL string) error {
	callbackURL = strings.TrimSpace(callbackURL)
	if callbackURL == "" {
		return Task.InsertWithContext(ctx)
	}
