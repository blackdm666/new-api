...
func TestRetiredFrontendAPIRoutes(t *testing.T) {
	...
	_, hasLegacyOptionUpdate := routes[http.MethodPost+" /api/option"]
	_, hasCurrentOptionUpdate := routes[http.MethodPut+" /api/option/"]
	assert.True(t, hasAsyncCleanup)
	assert.False(t, hasDirectDelete)
	assert.False(t, hasConsoleMigration)
	assert.True(t, hasLegacyOptionUpdate)
	assert.True(t, hasCurrentOptionUpdate)
}
