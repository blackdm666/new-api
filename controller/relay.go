...
func executeTaskSubmissionWith(
	...
	if insertErr := task.InsertWithCallbackContext(c.Request.Context(), relayInfo.CallbackURL, insertOmits...); insertErr != nil {
...
func presentTaskSubmission(c *gin.Context, outcome *taskSubmissionOutcome) {
	...
	if outcome.Result.ClientResponse != nil {
		diagnostics.present(outcome.Task, "legacy_presenter")
...
func decideTaskRetry(c *gin.Context, taskErr *taskdto.TaskError, retryTimes int) service.PolicyDecision {
	...
	case taskErr.StatusCode < 100 || taskErr.StatusCode > 599:
		retry.Reason = "unrecognized_status"
		return retry
	case operation_setting.IsAlwaysSkipRetryStatusCode(taskErr.StatusCode):
		stop.Reason = "system_retry_exclusion"
	case operation_setting.ShouldRetryByStatusCode(taskErr.StatusCode):
		retry.Source = "global"
		return retry
	default:
