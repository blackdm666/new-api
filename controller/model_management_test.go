		...
		t.Run(dialect.kind, func(t *testing.T) {
			...
			t.Run("explicit_fixed_units_survive_pricing_snapshot_writes", func(t *testing.T) {
				for _, mode := range []string{billing_setting.BillingModePerRequest, billing_setting.BillingModePerSecond} {
					...
					require.NoError(t, err)
					price := model.PricingValues{"ModelPrice": float64(0), "billing_setting.billing_mode": mode}
					require.NoError(t, model.UpdateModelPricing([]model.ModelPricingChange{{ModelName: name, ExpectedVersion: before.Entries[0].Version, Pricing: price}}))
					after, err := model.GetModelPricingSnapshot([]string{name})
					require.NoError(t, err)
					assert.Equal(t, price, after.Entries[0].Configured)
					assert.Equal(t, mode, billing_setting.GetBillingMode(name))
					assert.Error(t, model.ValidateModelPricing(name, model.PricingValues{"billing_setting.billing_mode": mode}))
				}
			})
