...
func TestSecurityEnrollmentOperationContext(t *testing.T) {
	for _, test := range []struct {
		...
		{"balance token", "channel.balance_token.read", `{"channel_id":123}`, nil},
		{"missing balance token channel", "channel.balance_token.read", ``, service.ErrVerificationContextInvalid},
...
func TestSecurityEnrollmentChannelProofRejectsMismatchesBeforeConsumption(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{"balance token scope", identity, service.VerificationOperation{Scope: "channel.balance_token.read", Context: operation.Context}, service.ErrProofScope},
