...
func mergeTaskBillingRatios(priceData *types.PriceData, ratios map[string]float64) {
	for name, ratio := range ratios {
		priceData.AddOtherRatio(name, ratio)
	}
}

func ensureTaskBillingRatios(c *gin.Context, modelName string, priceData *types.PriceData) error {
...
func validateTaskBillingRatios(modelName string, priceData types.PriceData) error {
	mode := billing_setting.GetBillingMode(modelName)
